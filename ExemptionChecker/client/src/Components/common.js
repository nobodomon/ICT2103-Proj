...
export class StdInput extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            initialSubValue: this.props.subValue,
            valueChanged: false,
        ...
        this.untoggleEdit = this.untoggleEdit.bind(this);
        this.feedback = this.feedback.bind(this);
        this.handleSave = this.handleSave.bind(this);
        this.handleSearchChange = this.handleSearchChange.bind(this);
        this.handleInternalFocus = this.handleInternalFocus.bind(this);
        this.setPrimaryInput = this.setPrimaryInput.bind(this);
        this.searchTagCancel = this.searchTagCancel.bind(this);
        this.searchTagEdit = this.searchTagEdit.bind(this);
    ...
    componentDidMount() {
        ...
        if (this.props.type === "text") {
            this.primaryInput.current.value = this.state.initialValue
        }
        ...
        if (this.props.type === "tel") {
            this.primaryInput.current.value = this.state.initialValue;
            this.subInput.current.value = this.state.initialSubValue;
        }
    }
    ...
    handleSave = (e, refs) => {
        ...
        if (this.props.type === "tel") {
            this.setState({
                initialValue: refs[0].current.value,
                initialSubValue: refs[1].current.value,
                valueChanged: false
            })
        } else {
    ...
    handleInternalFocus = (e) => {
        this.setState({
            internalFocus: "persistDropdown"
        })
    }
    ...
    setPrimaryInput(value, refs) {
        console.log(value);
        this.primaryInput.current.value = value;
        this.setState({
            valueChanged: true
        })
        if (this.primaryInput.current.value === this.state.initialValue) {
            this.setState({
                initialValue: this.primaryInput.current.value,
                valueChanged: false
            })
        }
        if (refs.includes(this.subInput)) {
            this.subInput.current.focus();
        }
    }
    ...
    tagSelectCallBack = (tag) => {
        ...
        this.props.tagSelectCallBack(tag);
    }

    onEditHandle = (e, ref) => {
        //this.props.onChange(e);
        if (this.props.isTextEmpty) {
            this.props.isTextEmpty(e);
        }
        ...
        if (this.props.type === "tel") {
            ...
            if (ref === this.subInput) {
                if (e.target.value === this.state.initialSubValue) {
                    this.setState({
                        initialSubValue: e.target.value,
                        valueChanged: false
                    })
                }
            ...
            if (ref.current.value === this.state.initialValue) {
                this.setState({
    ...
    resetFlash() {
        ...
        element.classList.remove("inputFlash")
    }

    render() {
        return (
            <div className="d-flex flex-column flex-md-row justify-content-stretch stdInputContainer" id={this.props.id} onAnimationEnd={this.resetFlash}>
                {this.props.label &&

                    <label className="stdInputLabel">{this.props.label}</label>
                }
                <div className="d-flex align-items-center">
                    ...
                    {this.props.type === "tel" &&
                        <div onClick={() => this.toggleEdit()} className={"stdInputGroup d-flex align-items-center" + " " + (this.state.valueChanged ? "leftBorderRadius" : "borderRadius")} >
                            ...
                            <input className="stdInput sub" style={{ "--width": "75px" }} type={"dropdown"} disabled={!this.state.editable} ref={this.primaryInput} autoComplete={this.props.autoComplete} placeholder={""} onChange={(e) => { this.onEditHandle(e, this.primaryInput); this.handleSearchChange(e) }}>
                            </input>
                            <div className="dropdownWrapper" style={{ "--maxItems": 5 }}>
                                <div className={"dropdown"}>
                                    {telCodes.map((option, index) => {

                                        if (this.state.searchQuery === "" || option.label.toLowerCase().includes(this.state.searchQuery.toLowerCase())) {
                                            return <StdInputCountryCodeDropDownOption key={index} value={option.value} type={option.type} countryCode={option.countryCode} onClick={() => this.setPrimaryInput(option.value, [this.primaryInput, this.subInput])}>{option.label}</StdInputCountryCodeDropDownOption>
                                        } else {
                                            return "";
                                        }
                                    })}
                            ...
                            <div className="dropdownWrapper">
                                <div className={"dropdown"}>
                                    {this.props.options.map((option, index) => {
                                        return <StdInputDropDownOption key={index} value={option.value} onClick={() => this.setPrimaryInput(option.value, [this.primaryInput])}>{option.label}</StdInputDropDownOption>
                                    })}
                                </div>
                                <div className="dropdown-spacer"></div>
                            </div>
                    ...
                    {this.props.type === "dropdown-tags" &&
                        <div onClick={() => this.toggleEdit()} className={"stdInputGroup d-flex align-items-center" + " " + (this.state.valueChanged ? "leftBorderRadius" : "borderRadius")}>
                            {this.state.selectedTag !== "" &&
                                <SearchTags onCancelClick={this.searchTagCancel} onEditClick={this.searchTagEdit} type={this.state.tagType}>{this.state.selectedTag} </SearchTags>
                            }
                            <input className="stdInput" type={"dropdown"} disabled={!this.state.editable} ref={this.primaryInput} autoComplete={this.props.autoComplete} placeholder={this.props.placeholder} onChange={(e) => this.onEditHandle(e, this.primaryInput)}>
                            </input>
                            <div className="dropdownWrapper">
                                <div className={"dropdown"}>
                                    {this.state.selectedTag === "" &&
                                        this.props.options.map((option, index) => {
                                            return <StdInputDropDownOption key={index} value={option.value} onClick={() => { this.tagSelect({ value: option.value, type: option.type }) }}>{option.value}</StdInputDropDownOption>
                                        })
                                    }
                                    {this.state.selectedTag !== "" &&
                                        this.props.options.map((option, index) => {
                                            return <StdInputDropDownOption key={index} value={option.value} onClick={() => { this.tagSelectCallBack({ value: this.state.selectedTag + "(" + option.value + ")", type: this.state.tagType }) }}>{option.value}</StdInputDropDownOption>
                                        })
                                    }
                                </div>
                                <div className="dropdown-spacer"></div>
                    ...
                    {this.props.showSaveBtn ? this.state.valueChanged ?
                        <div className="inputSave" onClick={(e) => { this.handleSave(e, [this.primaryInput, this.subInput]) }}><i class="bi bi-check-circle"></i></div> : "" : ""
                    }
                </div>
            </div>
        )
...
export class SegmentedInput extends React.Component {
